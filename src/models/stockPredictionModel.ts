...
class StockPredictionModel {
    ...
    async train(data: number[]): Promise<void> {
        ...
        // Train the model
        await this.model.fit(normalizedXs, normalizedYs, { epochs: 100 });
    ...
    predict(data: number): number {
        ...
        const normalizedData = (data - this.normalizationParameters.xsMin.dataSync()[0]) /
        ...
        const denormalizedPrediction = prediction.mul(this.normalizationParameters.ysMax.sub(this.normalizationParameters.ysMin)).add(this.normalizationParameters.ysMin);
