  ...
  async getCurrentPrice(symbol: string): Promise<number> {
    try {
      const response = await axios.get(
        `${this.baseUrl}/query?function=GLOBAL_QUOTE&symbol=${symbol}&apikey=${this.apiKey}`,
      )

      const price = parseFloat(response.data['Global Quote']['05. price'])
      return price
    } catch (error: any) {
      throw new Error(`Error fetching current price from Alpha Vantage: ${error.message}`)
    }
  }
}
